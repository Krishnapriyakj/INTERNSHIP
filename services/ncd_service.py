"""NCD assessment service for risk evaluation."""
from data.ncd_questions import NCD_QUESTIONS
...
class NCDAssessmentManager:
    """Manages NCD assessment workflow."""
    def __init__(self):
        self.user_sessions = {}

    def process_message(self, user_id, user_question):
        """Process a user message in the appropriate context (chat or assessment)."""
        ...
        if session["stage"] == "conversation":
            if any(keyword in user_question for keyword in ["ncd risk", "risk assessment", "health check", "assessment"]):
                ...
                # This will be handled by the chat service instead
                return None
        
        # Proceed with the assessment
        if session["stage"] == "assessment":
            return self._process_assessment_step(session, user_question)
    
    def _process_assessment_step(self, session, user_question):
        """Process a single step in the assessment workflow."""
